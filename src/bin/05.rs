...
pub fn part_two(input: &str) -> Option<u32> {
    ...
    updates
        ...
        .filter(|update| {
            // combinations will keep pairs in their relative ordering from the original vector
            // keep all updates with any invalid pair
            update.iter().combinations(2).any(|pair| {
                let first = pair[0];
                let second = pair[1];
                let invalid = rules.iter().any(|(x, y)| first == y && second == x);
        ...
        .map(|update| {
            ...
            // if a pair is invalid, swap them
            sorted_update.sort_by(|first, second| {
                if rules.iter().any(|(x, y)| first == y && second == x) {
